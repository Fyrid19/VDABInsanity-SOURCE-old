...
import grafex.effects.GlitchEffect;
...
class PlayState extends MusicBeatState
{
	...
	var dave3dshader:GlitchEffect = new GlitchEffect();
	...
	override public function create()
	{
		...
		if (SONG == null)
			SONG = Song.loadFromJson('tutorial', 'charts');
		...
		switch (curStage)
		{
			case 'morrowBG':
				...
				dave3dshader = new GlitchEffect();
				dave3dshader.waveSpeed = 2;
				dave3dshader.waveFrequency = 5;
				dave3dshader.waveAmplitude = 0.1;
                bg.shader = dave3dshader.shader;
				curbg = bg;
			...
			case 'scarybg':
				defaultCamZoom = 0.65;
				var bg:FlxSprite = new FlxSprite(200, 0).loadGraphic(Paths.image('backgrounds/scarybg'));
	...
	private function generateSong(dataPath:String):Void
	{
		...
		#if MODS_ALLOWED
		if (FileSystem.exists(Paths.modsJson('events/' + songName + '/events')) || FileSystem.exists(file)) {
		#else
	...
	var camFocus:String = "";
	var daFunneOffsetMultiplier:Float = 40;
	...
	function getCutsceneFiles()
	{
		...
		var file:String = Paths.json('dialogue/' + songName + '/dialogue'); //Checks for json/Psych Engine dialogue
		if (OpenFlAssets.exists(file)) {
			dialogueJson = DialogueBoxPsych.parseDialogue(file);
		}

		var file:String = Paths.json('cutscenes/' + songName + '/cutscene'); //Checks for json/Grafex cutscene
		if (OpenFlAssets.exists(file)) {
			cutsceneJson = Cutscene.parseCutscene(file);
		}

		var file:String = Paths.json('cutscenes/' + songName + '/contCutscene'); //Checks for json/Grafex cutscene
		if (OpenFlAssets.exists(file)) {
			cutsceneJsonCont = Cutscene.parseCutscene(file);
		}

		var file:String = Paths.txt('dialogue/' + songName + '/' + songName + 'Dialogue'); //Checks for vanilla/Senpai dialogue
		if (OpenFlAssets.exists(file)) {
